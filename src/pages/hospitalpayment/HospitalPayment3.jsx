...
const HospitalPayment = () => {
  ...
  const filteredReason = reasons
    ...
    .filter((r) => r.toLowerCase().includes(searchText.toLowerCase()));
  ...
  useEffect(() => {
    const fetchReasons = async () => {
      try {
        ...
        if (response?.status === 200) {
          setReasons(
            response?.data?.map(({ purpose, amount }) => ({ purpose, amount }))
          );
        }
      } catch (error) {
                    ...
                    <Chip
                      ...
                      onDelete={(e) => {
                        ...
                        setFormData((prev) => {
                          ...
                          return { ...prev, reason: updatedReason,amount: prev.amount.filter((item)=>item.purpose !== value) };
                        });
                      }}
                    />
                  ))}
