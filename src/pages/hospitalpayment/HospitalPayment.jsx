...
export const generatePDF = (data) => {
	try {
		...
		data?.amount?.forEach((item) => {
			...
			drawText(`${formatAccounting2(parseFloat(item.amount).toFixed(2))}`, marginRight - 25, yPos);
			yPos += lineHeight;
		});
		...
		const totalAmount = data?.amount?.map((item) => parseFloat(item.amount)).reduce((a, b) => a + b, 0);

		doc.setFont('NotoSansEthiopic-Regular', 'normal');
		drawText(`Total In Figure`, marginLeft, yPos);
		drawText(`${formatAccounting2(totalAmount.toFixed(2))}`, marginRight - 25, yPos);
		yPos += lineHeight;

		drawText(`Total In Words : `, marginLeft, yPos);
		drawText(`${capitalizeWords(numberToWords.toWords(totalAmount.toFixed(2)))} birr`, marginLeft + 30, yPos, {
		...
		doc.setFont('NotoSansEthiopic-Regular', 'normal');
		drawText('This Receipt is invalid unless it is stamped.', pageWidth / 2, yPos, { align: 'center' });
...
const HospitalPayment = () => {
	...
	const checkboxColor = theme.palette.mode === 'light' ? theme.palette.primary.main : theme.palette.secondary.main;
		...
		const fetchPaymetInfo = async () => {
			try {
				...
				if (response.status === 200) {
					const sortedPayment = await response?.data.sort((a, b) => b.rowId - a.rowId);
	...
	useEffect(() => {
		const fetchMeth = async () => {
			try {
				...
				if (response?.status === 200) {
					setPaymentMehods(response?.data?.filter((item) => item.type !== 'ALL').map((item) => item.type));
				}
			} catch (error) {
				console.error(error.message);
			}
	...
	const handleAmountChange = (e, reason) => {
		try {
			...
			setFormData((prev) => {
				...
				const finalAmounts = updatedAmount.some((item) => item.purpose === reason)
					? updatedAmount
			...
			setFormDataError((prev) => {
				const finalAmounts = prev?.amount.some((item) => item.purpose === reason)
					? prev?.amount?.filter((item) => item.purpose !== reason)
					: [...prev?.amount];
	...
	const handleSubmit = () => {
		try {
			const amountErrormsg = 'Each reason must have a corresponding payment amount greater than 0!';
			...
			const isAmountValid = formData.reason.every((reason) =>
				formData.amount.some((item) => item.purpose === reason && item.amount > 0)
			);
			...
			if (!isAmountValid) {
				const isNew = formData.reason?.map((item) => {
					if (
						...
						formData?.amount?.filter((k) => k.purpose === item)?.every((ite) => ite.amount === 0)
					) {
			...
			<Paper sx={{ padding: 2, marginBottom: 2, display: 'flex', gap: 2 }}>
				<Box sx={{ flex: 1 }}>
					<TextField
						...
						fullWidth
						error={cardNumberError?.length > 0 ? !!cardNumberError : !!formDataError?.cardNumber}
						helperText={cardNumberError?.length > 0 ? cardNumberError : formDataError?.cardNumber}
						margin="normal"
						variant="outlined"
						sx={{
						...
						FormHelperTextProps={{
							style: {
								color: !!cardNumberError || !!formDataError?.cardNumber ? 'red' : 'green',
								fontSize: '14px',
							},
						}}
					/>
					...
					{formData?.reason?.map((reason, index) => (
						<TextField
							...
							required
							margin="normal"
							value={formData?.amount?.find((item) => item.purpose === reason)?.amount || ''}
				...
				<Box sx={{ flex: 1 }}>
					<Paper sx={{ padding: 2, marginBottom: 2 }}>
						...
						{paymentSummary.map((summary) => (
							...
							>
								<Typography sx={{ fontWeight: 'bolder' }}>{summary.method}</Typography>
						...
						>
							...
							<Typography>
								{formatAccounting(
									paymentSummary.map((e) => e.amount).reduce((acc, num) => acc + num, 0)
								)}
							</Typography>
						</Box>
					</Paper>
