...
const keysToRemoveByPaymentType = {
	...
	cbhi: ['carPlateNumber', 'policePhone', 'policeName', 'accedentDate', 'patientWorkingPlace', 'patientWorkID'],
	credit: ['carPlateNumber', 'policePhone', 'policeName', 'accedentDate', 'cbhiProvider', 'idNo', 'referalNo'],
	traffic: ['cbhiProvider', 'patientWorkID', 'idNo', 'referalNo', 'patientWorkingPlace'],
};
...
const exportToPDF = async (data) => {
	try {
		...
		const widthDistribution = [2, 4, 10, 7, 5, 3, 4, 4, 4, 4, 5, 5, 6, 5, 6, 6, 6, 5, 5, 4, 4, 4];
		...
		autoTable(doc, {
			...
			didDrawPage: function () {
				...
				doc.text('Page ' + doc.internal.getNumberOfPages(), pageWidth - margin, pageHeight - 20, {
					align: 'right',
				});
			},
		});
...
const ReportPage = () => {
	...
	useEffect(() => {
		try {
			...
			const prev = new Set(paymentMethods.map((item) => item?.type?.toLowerCase()));
			const diffSet = new Set([...newSet].filter((x) => !prev.has(x?.toLowerCase())));

			if (diffSet.size === 0) return;

			const maxId = paymentMethods?.length > 0 ? Math.max(...paymentMethods.map((item) => item.id)) : 0;
	...
	const exportToExcel = async () => {
		try {
			...
			XLSX.writeFile(wb, `Payments_Report_${startDate}_to_${endDate}_${selectedMethod}.xlsx`);
		} catch (error) {
			...
			setFilteredPayments(
				filteredRows
					? filteredRows?.filter((payment) =>
							selectedMethod === 'ALL'
								? payment
								: payment.paymentType.toLowerCase() === selectedMethod.toLocaleLowerCase()
					  )
					: []
			);
		}
	...
	const calculateTotal = (method) => {
		try {
			return filteredRows
				? filteredRows
						?.filter((payment) =>
							method === 'ALL' ? payment : payment.paymentType?.toLowerCase() === method?.toLowerCase()
						)
				...
				setFilteredPayments(
					filteredRows
						? filteredRows?.filter((payment) =>
								selectedMethod === 'ALL'
									? payment
									: payment.paymentType.toLowerCase() === selectedMethod.toLowerCase()
						  )
						: []
				);
			}
	...
	const filterData = (name, value) => {
		try {
			if (name === 'woreda') {
				...
				setFilteredPayments(copy?.filter((item) => item.kebele?.toLowerCase() === value.toLowerCase()));
			} else if (name === 'organization') {
				const copy = filteredRows;
				setFilteredPayments(
					copy?.filter((item) => item.patientWorkingPlace?.toLowerCase() === value.toLowerCase())
				);
			}
		} catch (error) {
	...
	return (
		<Box>
				...
				<Grid container columnSpacing={1} rowSpacing={2}>
					...
					<Grid item xs={12} sm={6} md={2}>
						<Button
							...
							// sx={{ height: "100%" }}
						>
							{isLoading ? <CircularProgress size={24} color="inherit" /> : 'Request Report'}
						</Button>
					</Grid>
					...
					<Grid item xs={12} sm={6} md={4}>
						<FormControl fullWidth>
							...
							<Select value={operator} label="Filter Type" onChange={(e) => handleOperatorChange(e)}>
			...
			<Button sx={{ marginLeft: 2 }} variant="contained" color="primary" onClick={() => exportToExcel()}>
				Export to Excel
			</Button>
			<Button sx={{ marginLeft: 2 }} variant="contained" color="primary" onClick={() => cumulativeReport()}>
				Cumulative Export
			</Button>

			<Button sx={{ marginLeft: 2 }} variant="contained" color="primary" onClick={() => handlexportToPDF()}>
				Export to PDF
			</Button>
